  ...
  async checkPatterns() {
    try {
      ...
      if (this.checks.pattern) {
        ...
        /* This extra check will work as long as Wazuh monitoring index ID is EH-Monitoring-*.
           ...
           */
          try{
              await SavedObject.existsMonitoringIndexPattern('EH-Monitoring-*'); //this checks if it exists, if not it automatically creates the index pattern
          }catch(err){}
  ...
  async load() {
    try {
      ...
      this.results.push(
        {
          id: 0,
          description: 'Check EH-ISMAC API connection',
          status: this.checks.api ? 'Checking...' : 'disabled'
        },
        {
          id: 1,
          description: 'Check for EH-ISMAC API version',
          status: this.checks.setup ? 'Checking...' : 'disabled'
        },
        {
          id: 2,
          description: 'Check EH-ISMAC index pattern',
          status: this.checks.pattern ? 'Checking...' : 'disabled'
        },
        {
          id: 3,
          description: 'Check EH-ISMAC template',
