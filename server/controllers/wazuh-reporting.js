    ...
    this.dd = {
      ...
      header: {
        ...
        columns: [
          ...
          {
            text: 'info@exploithunters.com\nhttps://exploithunters.com',
            alignment: 'right',
            margin: [0, 0, 40, 0],
            color: '#000000'
          }
      ...
      footer(currentPage, pageCount) {
        return {
          columns: [
            {
              text: 'Copyright © 2020 Exploit Hunters Segurança da Informação, LDA.',
              color: '#000000',
              margin: [40, 40, 0, 0]
            },
            {
  ...
  async report(req, reply) {
    try {
      ...
      if (req.payload && req.payload.array) {
        ...
        if (!apiId)
          throw new Error(
            'Reporting needs a valid EH-ISMAC API ID in order to work properly'
          );
        if (!name)
          throw new Error(
