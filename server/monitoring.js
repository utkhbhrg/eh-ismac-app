...
export class Monitoring {
  ...
  constructor(server, quiet = false) {
    ...
    this.CRON_FREQ = '0 1 * * * *';
    this.CREATION = 'd';
    this.index_pattern = 'EH-Monitoring-*';
  ...
  async checkTemplate() {
    try {
      ...
      try {
        ...
        monitoringTemplate.index_patterns = ['EH-Monitoring-*'];
      }
    ...
    try {
      ...
      try {
        !this.quiet &&
          log(
            'monitoring:init',
            'Checking if EH-Monitoring-* index pattern exists...',
            'debug'
          );
        ...
        const didNotFindMsg =
          "Didn't find EH-Monitoring-* index pattern for Kibana. Proceeding to create it...";
